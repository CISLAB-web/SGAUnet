...
def train():
    ...
    if select_model== "CustomUnet": # use gate attention
        ...
        model = ResNet50Regression(num_outputs=4).to(device)
    elif select_model=="UNet":
        model = UNetRegression(num_outputs=4).to(device)
    elif select_model=="DenseNet121":
        model = DenseNet121Regression(num_outputs=4).to(device)
    elif select_model== "AttUNet":
        model = AttUNetRegression(num_outputs=4).to(device)
    elif select_model== "SwinTransformer":
        model = SwinTransformerRegression(num_outputs=4).to(device)
    elif select_model== "ViTRegression":
        model = ViTRegression(num_outputs=4).to(device)
    elif select_model == "UKAN":
        model = UKANRegression(num_outputs=4, img_size=256).to(device)
    elif select_model == "SwinUNet":
        model = SwinUNetRegression(num_outputs=4, img_size=256).to(device)
    elif select_model == "UTANet":
        model = UTANetRegression(num_outputs=4, img_size=256, use_tamosc=True).to(device)
    elif select_model == "MK_UNet_S":
        model = MKUNetSRegression(num_outputs=4).to(device)
    else:
        raise "Model type must in 'ViTRegression, SwinTransformer, SwinUNet, UTANet, MK_UNet_S, ResNet50, UNet, DenseNet121, AttUNet, CustomUnet*, UKAN'"
    ...
    # 审稿消融：Gaussian / MDM-only / WDM-only 伪标签仅在双头模型 CustomUnet_att_ca 上生效
    ...
    if use_att_ca_pseudo_modes:
        ...
        logger.info(
            f"model={select_model}: `seg_supervision` (Gaussian / gmdm / gwdm) applies only "
    ...
    for epoch in range(start_epoch, 101):
        ...
        for batch in tqdm(train_loader, desc=f"Training Epoch {epoch}"):
            ...
            if reg_output is not None:
                loss_reg = criterion_regression(reg_output, targets / 100.0)
                total_loss += loss_reg
