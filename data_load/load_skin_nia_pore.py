...
def process_single_image(image_block):
    # 转换为灰度图
    ...
    # 高斯滤波去噪
    blurMat = cv2.GaussianBlur(enhanced_gray, (3, 3), 0)

    # 使用自适应阈值进行二值化
    thresMat = cv2.adaptiveThreshold(blurMat, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY_INV, 21, 5)

    # 形态学操作去噪
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    morphed = cv2.morphologyEx(thresMat, cv2.MORPH_OPEN, kernel)
    ...
    # 筛选轮廓面积
    contours_sel = [contour for contour in contours if 20 < cv2.contourArea(contour) < 500]

    # 绘制轮廓
    # 创建黑色背景块
    output_block = np.zeros_like(image_block)  # 创建黑色背景
    # result_image = cv2.drawContours(image_block.copy(), contours_sel, -1, (0, 255, 0), 1)
    # thickness=-1，这样可以填充轮廓的内部区域
    result_image = cv2.drawContours(output_block.copy(), contours_sel, -1, (255, 255, 255), -1)
    return result_image, len(contours_sel)
...
class CustomDataset_skin_level(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # 调用检测毛孔方法
            pores_map , pores_count = process_single_image(cropped_img)
            pores_map = cv2.cvtColor(pores_map, cv2.COLOR_BGR2GRAY)  # 转换为灰度图

            # 将裁剪的图像和毛孔检测图都转换为 PIL 格式
            cropped_img_pil = Image.fromarray(cropped_img)
            pores_map_pil = Image.fromarray(pores_map).convert('RGB')  # 转为伪 RGB 图像

            # 定义需要排除的变换类型
            exclude_types = [transforms.Normalize, transforms.ColorJitter]
    
            if self.transform:
                # 内联过滤变换
                filtered_transform = transforms.Compose([
                    t for t in self.transform.transforms if not isinstance(t, tuple(exclude_types))
                ])
                # cropped_img = self.transform(cropped_img)
                # 确保相同随机变换
                ...
                pores_map_transformed = filtered_transform(pores_map_pil)
            ...
            labels = {
                "skin_type": skin_type,
                "pores_count": torch.tensor(pores_count, dtype=torch.float),  # 毛孔计数

            }
    
            return [cropped_img_transformed, pores_map_transformed, labels]
